...
#work_file_dir="/Users/masen/Desktop/HLA_work/hla_typing_result/results_summary/merge_results_scripts_and_accuracy/accuracy_summary/KGP_summary/"
...
with open(work_file_dir+"/HLA_LA_summary.txt")as f:
    for i in f.readlines():
        id=i.rstrip().split("\t")[0]
        part_dict_answers_to_match.append(id)
with open(work_file_dir+"/kourami_summary.txt")as f:
    for i in f.readlines():
        id=i.rstrip().split("\t")[0]
        part_dict_answers_to_match.append(id)
...
for type in type_to_call:
    #dict_1 = get_hla(work_file_dir+"/hla_genotype_summary.txt",type)
    dict_2 = get_hla(work_file_dir+"/hlahd_summary.txt",type)
    dict_3 = get_hla(work_file_dir+"/optitype_summary.txt",type)
    dict_4 = get_hla(work_file_dir+"/HLA_LA_summary.txt",type)
    dict_5 = get_hla(work_file_dir+"/kourami_summary.txt", type)
    for sample in part_dict_answers_to_match:
        result1 = [dict_3[sample][0],dict_4[sample][0],dict_5[sample][0]]
        result2 = [dict_3[sample][1],dict_4[sample][1],dict_5[sample][1]]
    ...
    for sample in part_dict_answers_to_match:
        result3 = [dict_2[sample][0],dict_4[sample][0],dict_5[sample][0]]
        result4 = [dict_2[sample][1],dict_4[sample][1],dict_5[sample][1]]
...
def turn_none_to_kong(alsit):
    final=[" " if not i else i  for i  in alsit]
    return final
